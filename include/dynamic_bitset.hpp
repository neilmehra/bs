...
//     return std::any_of(data.begin(), data.end(), [](const auto &v) {
//       v != 0;
//     });
//   }
//
...
//
//   bitset& set_unchecked(std::size_t pos, bool val = true);
//
...
// };
//
// template <std::size_t N> class bitset::reference {
// public:
//   friend class bitset;
//
//   reference() = delete;
...
// };
//
// template <std::size_t N> bitset& bitset::set_unchecked() {
//   block_t mask = std::numeric_limits<block_t>::max();
//   for (std::size_t i = 0; i < num_blocks; i++) {
//     data[i] |= mask;
//   }
...
// bitset& bitset::set_unchecked(std::size_t pos, bool val) {
//   if (!val)
...
// }
//
// template <std::size_t N> bitset& bitset::reset_unchecked() noexcept {
//   for (std::size_t i = 0; i < num_blocks; i++) {
//     data[i] &= 0;
//   }
//   return *this;
...
//
// template <std::size_t N>
// bitset& bitset::reset_unchecked(std::size_t pos) {
//   std::size_t block_idx = pos / block_t_bitsize;
//   std::size_t bit = pos - (block_t_bitsize * block_idx);
//   block_t mask = std::numeric_limits<block_t>::max();
...
// operator<<(std::basic_ostream<charT, traits>& os, const bitset& x) {
//   os << x.to_string();
//   return os;
// };
//
